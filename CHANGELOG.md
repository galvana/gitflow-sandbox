# Changelog

All notable changes are compiled here from `changelog/` fragments at release time
by `nox -s "changelog(write)" -- --release VERSION`.

## [Unreleased](https://github.com/galvana/gitflow-sandbox/compare/2.3.2..main)

## [2.3.2](https://github.com/galvana/gitflow-sandbox/compare/2.3.1..2.3.2)

### Fixed
- Hotfix 2.3.2 for revised-strategy test [#17](https://github.com/galvana/gitflow-sandbox/pull/17)

## [2.3.1](https://github.com/galvana/gitflow-sandbox/compare/2.3.0..2.3.1)

### Fixed
- Critical hotfix for sandbox 2.3.1 test [#11](https://github.com/galvana/gitflow-sandbox/pull/11)

## [2.3.0](https://github.com/galvana/gitflow-sandbox/compare/2.2.0..2.3.0)

### Added
- v2.3 feature for integrated sandbox test [#8](https://github.com/galvana/gitflow-sandbox/pull/8)

## [2.2.0](https://github.com/galvana/gitflow-sandbox/compare/2.0.0..2.2.0)

### Added
- Changelog demo feature for sandbox release test [#5](https://github.com/galvana/gitflow-sandbox/pull/5)

## [2.0.0](https://github.com/galvana/gitflow-sandbox/releases/tag/2.0.0)

- Initial seed.
